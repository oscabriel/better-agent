...
**Curator**:
The thin product-level agent role, one per user, that helps users create, find, configure, and route between Thinkspaces, and that protects the user's Attention by maintaining and surfacing the cross-Thinkspace Review Queue. The Curator shapes each new Thinkspace through a live conversation that proposes the draft's Agent Profile — Goal, instructions, model, and enabled tools with their requested Permissions — but never grants a Permission and never activates; both remain the user's act.
...
- The **Curator** helps create and route between **Thinkspaces**; creating a **Thinkspace** happens only through a live conversation with the **Curator**, which shapes the draft as the user watches it take form.
- The **Curator** reasons on the user's own model credential to shape a draft, separately from the model it configures for the **Thinkspace Agent**; this crosses no **Thinkspace** boundary because no **Thinkspace Agent** is yet running and the **Permission** that gates a Thinkspace Agent's use of that credential does not yet exist.
- A connected-account tool the **Curator** proposes is subject to three distinct gates: the **Connected Account** must exist at the product level, the **Thinkspace** must grant the **Permission** at activation, and each action still needs an **Approval** at runtime.
...
- "Coordinator" and "curator" both described the setup/routing role; resolved: the role is renamed **Curator** (ADRs 0001 and 0005 predate the rename and still say "Coordinator"). Guided Thinkspace creation is a Curator capability, not a separate role — and the implementation question is now resolved (ADR-0010): for creation it runs as a durable, per-draft conversational agent that proposes the draft's configuration but never grants Permissions, while the per-user routing and Review Queue responsibilities remain a separate future capability of the same role.
- The live conversation in which the user shapes a draft with the **Curator** could be mistaken for a **Sitting**; resolved: a **Sitting** is owner ↔ **Thinkspace Agent** over an active Thinkspace's durable history, while Curator-led creation is the **Curator** shaping a draft before any **Thinkspace Agent** runs — keep **Sitting** for the working session, not for creation.
...
- "Connected Account" and the credential that backs it blurred during the external-mutation design (PRD #108); resolved: a **Connected Account** is the product-level relationship, while the stored, encrypted credential merely backs it. The agent never receives the credential; it is read only at tool-execution time, after an **Approval** (ADR-0009). The Connected Account is also distinct from a Project Think MCP server connection, which is an implementation concept (ADR-0006), not a product term.
